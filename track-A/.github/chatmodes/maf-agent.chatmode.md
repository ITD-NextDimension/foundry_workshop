---
description: 'Microsoft Agent Framework + Foundry hosted agent vibe-coding mode for the workshop track-A.'
...
This is `workshop/track-A/` of the foundry workshop. The shared Foundry resources (account + project + model deployment + ACR) are already pre-deployed by the instructor in a separate repo. The learner only deploys **their own hosted agent** (named with a `STUDENT_SUFFIX`) into the shared project.
...
- The hosted agent responses URL is `/agents/<name>/endpoint/protocols/openai/responses?api-version=2025-11-15-preview` — not `/agents/<name>/responses`. Use `workshop-scripts/invoke-hosted.ps1` to call.
...
- Re-create infra bicep — `workshop/track-A/infra/` was removed; the shared Foundry is pre-deployed.
