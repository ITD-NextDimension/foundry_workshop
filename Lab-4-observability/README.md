...
> 一个单文件 HTML + 一个 PowerShell 脚本，凭 SP 直接调 Foundry agents 数据平面 API 拉自己 hosted agent 的运行数据。
...
| `data/my-traces.json` | `fetch-traces.ps1` 写出的本人 trace (gitignore) |
...
`fetch-traces.ps1` 用根目录 `.env` 里的 SP 凭据走 OAuth2 client_credentials 拿 token (audience `https://ai.azure.com/.default`)。对应 SP 已经在共享 project 上被讲师授予 `Cognitive Services User`。无需 portal、不需要 Log Analytics / App Insights 权限。
