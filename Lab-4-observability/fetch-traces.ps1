...
.SYNOPSIS
    ...
    供 Lab-4-observability/index.html 渲染。
...
# ---------------------------------------------------------------------------
# 1. 读 .env (workshop 根), 拿 endpoint / agent name / OAuth2 token
# ---------------------------------------------------------------------------
...
if (-not $endpoint) { throw "AZURE_AI_PROJECT_ENDPOINT 未设置 (.env)." }
if (-not ($clientId -and $clientSecret -and $tenantId)) {
    throw "SP 凭据 (AZURE_CLIENT_ID/SECRET/TENANT_ID) 未在 .env 设置."
}
...
try {
    ...
    throw "OAuth2 token 拿不到: $($_.Exception.Message)"
}
if (-not $token) { throw "OAuth2 返回空 token." }
