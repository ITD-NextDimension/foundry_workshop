...
|------|------------|------|
| 生成 / 修 agent.yaml + manifest | `/deploy agentDir=… agentDeployName=…` | 复制 `track-A/.github/prompts/deploy.prompt.md` 的模板 → `gh copilot suggest` |
...
```powershell
$tpl = Get-Content track-A\.github\prompts\deploy.prompt.md -Raw
