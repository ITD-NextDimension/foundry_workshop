...
.DESCRIPTION
    workshop 仓库的 `track-A/.github/{chatmodes,instructions,prompts}/` 已经放好，
    本脚本仅:
...
# 切到 track-A 目录 (chatmodes / prompts 在那)
$trackA = Join-Path $WorkspaceRoot 'track-A'
if (-not (Test-Path $trackA)) {
    throw "找不到 $trackA。请把脚本放在 workshop/workshop-scripts/ 下并从 workshop/ 目录跑。"
}
...
$skills = @(
    @{ Type = 'chatmode';     Path = 'track-A/.github/chatmodes/maf-agent.chatmode.md';        Trigger = 'Copilot Chat 顶部下拉 → "maf-agent"' },
    @{ Type = 'instructions'; Path = 'track-A/.github/instructions/maf-tools.instructions.md'; Trigger = '编辑 tools/**/*.py 时自动注入' },
    @{ Type = 'instructions'; Path = 'track-A/.github/instructions/maf-personas.instructions.md'; Trigger = '编辑 personas/**/*.md 时自动注入' },
    @{ Type = 'instructions'; Path = 'track-A/.github/instructions/maf-skills.instructions.md'; Trigger = '编辑 skills/**/SKILL.md 时自动注入' },
    @{ Type = 'prompt';       Path = 'track-A/.github/prompts/persona.prompt.md'; Trigger = 'Chat 输入 /persona' },
    @{ Type = 'prompt';       Path = 'track-A/.github/prompts/skill.prompt.md';   Trigger = 'Chat 输入 /skill' },
    @{ Type = 'prompt';       Path = 'track-A/.github/prompts/tool.prompt.md';    Trigger = 'Chat 输入 /tool' },
    @{ Type = 'prompt';       Path = 'track-A/.github/prompts/deploy.prompt.md';  Trigger = 'Chat 输入 /deploy' }
)
foreach ($s in $skills) {
