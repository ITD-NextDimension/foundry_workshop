...
## 3.3 用 Copilot 检查 / 生成 yaml

切到 `maf-agent` chatmode，输入：
...
Copilot 会按 `.github/prompts/deploy.prompt.md` 重写 `agent.yaml` / `agent.manifest.yaml`，并保证：
...
## 3.4 注册进 azd（覆盖 Lab 1 placeholder）
...
如果 Lab 1 注入的 `agent-framework-agent-basic-responses` 还在 `azure.yaml.services` 里，删掉它：手动编辑 `azure.yaml`，只留 `research-agent` 这一段。
...
```powershell
..\workshop-scripts\invoke-hosted.ps1 -StatusOnly -AgentName "research-agent-$env:STUDENT_SUFFIX"
...
→ [Lab 4 · 本地可观测性](lab-4-observability.md)
