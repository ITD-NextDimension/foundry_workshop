...
- **GitHub Copilot + MAF skills** 是把概念变代码的力放大器：`/persona` `/skill` `/tool` `/deploy` 四把斧头。
...
### Phase 3：评估闭环（workshop 没跑）

把 trace 转成评估数据集 → 跑 batch eval → 比较版本 → `prompt_optimize` → redeploy：

- 参考 `agent-observability-evaluation.md`：
  - §5 Evaluators 选型
...
参考 `agent-harness-architecture.md` §6。
...
参考 `agent-observability-evaluation.md` §11。
