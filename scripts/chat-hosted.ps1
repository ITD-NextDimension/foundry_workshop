...
.DESCRIPTION
    流程:
        ...
        4. 在默认浏览器中打开 scripts/chat-hosted/index.html
    ...
    凭据来源优先级 (越靠前优先级越高):
        ...
        3. Lab-2-vibe-coding/.env (KEY=VALUE 行)
...
.PARAMETER EnvFile
    从哪读 .env, 默认 .\.env (相对当前目录, 学员一般在 Lab-2-vibe-coding 里跑)。
...
.EXAMPLE
    # 推荐: 在 Lab-2-vibe-coding 目录, .env 已填好 SP + endpoint
...
#    Default: workshop root (one level above this script's folder).
# ---------------------------------------------------------------------------
...
if ($missing.Count -gt 0) {
    ...
    Write-Host "   填到 Lab-2-vibe-coding\.env 里, 或显式传 -ClientId/-ClientSecret/-TenantId/-Endpoint 参数。" -ForegroundColor DarkGray
